  ...
  <div class="manage-container">
    ...
    <div class="selector">
      <select ref="mainSlideSelector" @change="onSlideSelect">
        <option value="-1">이미지를 선택해 주세요</option>  
        <option
...
@Component({
  name: "AdminFellowshipPeople",
  components: {
    MainSlideView
  }
})
export default class AdminFellowshipPeople extends Vue {
  ...
  private async onDelete(index: number) {
    try {
      if (confirm("삭제하시겠습니까?")) {
        await AxiosHelper.POST("/deleteFellowshipPeople.php", {
          index: index
        });
