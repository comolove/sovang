...
export default class AdminCateringMainSlide extends Vue {
  ...
  async LoadData() {
    try {
      const { data } = await AxiosHelper.GET("/getCateringMainSlides.php");
      const list = data.data;

      this.mainSlides = list;
    } catch (error) {
      alert("케이터링 메인 슬라이드 로딩 실패");
      console.log(error);
    }
  }
  ...
  private async onDelete(index: number) {
    try {
      if (confirm("삭제하시겠습니까?")) {
        await AxiosHelper.POST("/deleteCateringMainSlide.php", {
          index: index
        });
